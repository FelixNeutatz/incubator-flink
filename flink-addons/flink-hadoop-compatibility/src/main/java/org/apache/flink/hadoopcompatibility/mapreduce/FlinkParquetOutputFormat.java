...
public class FlinkParquetOutputFormat<K,V> implements OutputFormat<Tuple2<K, V>> {
	...
	public FlinkParquetOutputFormat(org.apache.hadoop.mapreduce.OutputFormat<K, V> mapreduceOutputFormat, Job job) {
		super();
	...
	public void writeRecord(Tuple2<K, V> record) throws IOException {
		try {
			this.recordWriter.write(null, record.f1);
		} catch (InterruptedException e) {
